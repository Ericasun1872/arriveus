import type { Metadata } from "next";
import Link from "next/link";
import { siteConfig } from "@/lib/site";

export const metadata: Metadata = {
  title: "채용·광고 문의",
  description:
    "ArriveUS 채용 공고 게시판은 검수 후 운영 예정입니다. 지금은 취업 가이드를 이용하고, 게재·광고 문의는 이메일로 보내 주세요.",
  alternates: { canonical: "/jobs" },
  robots: { index: false, follow: true },
};

export default function JobsPage() {
  const email = siteConfig.contactEmail;
  const mailHref = `mailto:${email}?subject=${encodeURIComponent(
    "ArriveUS 채용 광고 문의",
  )}`;

  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
      <nav className="mb-6 text-sm text-[var(--muted)]">
        <Link href="/" className="hover:text-[var(--brand)]">
          홈
        </Link>
        <span className="mx-2">/</span>
        <Link href="/guides/job" className="hover:text-[var(--brand)]">
          취업
        </Link>
        <span className="mx-2">/</span>
        <span className="text-[var(--ink)]">채용·광고 문의</span>
      </nav>

      <header className="animate-rise">
        <p className="text-sm font-medium text-[var(--brand)]">Hiring</p>
        <h1 className="mt-1 font-[family-name:var(--font-display)] text-3xl font-semibold text-[var(--ink)] sm:text-4xl">
          채용 공고는 준비 중
        </h1>
        <p className="mt-3 text-sm leading-relaxed text-[var(--muted)]">
          빈 목록을 보여 드리기보다, 검수된 공고가 생길 때까지 취업 가이드와
          이메일 문의로 안내합니다. ArriveUS는 채용 알선·이민 대행을 하지
          않습니다.
        </p>
      </header>

      <div className="mt-8 rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)]/60 px-5 py-6">
        <p className="text-sm font-semibold text-[var(--ink)]">일자리가 급하면</p>
        <p className="mt-1 text-sm leading-relaxed text-[var(--muted)]">
          온라인 구직·현장·홈케어·이력서 가이드를 먼저 이용하세요. Worker.gov
          임금 사기·산재 안내도 취업 카테고리에 있습니다.
        </p>
        <div className="mt-4 flex flex-wrap gap-3">
          <Link
            href="/guides/job"
            className="inline-flex rounded-lg bg-[var(--brand)] px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90"
          >
            취업 가이드
          </Link>
          <Link
            href="/guides/job/job-search"
            className="inline-flex rounded-lg border border-[var(--border)] px-4 py-2.5 text-sm font-semibold text-[var(--ink)] hover:bg-[var(--surface)]"
          >
            온라인 구직
          </Link>
          <Link
            href="/guides/job/home-care"
            className="inline-flex rounded-lg border border-[var(--border)] px-4 py-2.5 text-sm font-semibold text-[var(--ink)] hover:bg-[var(--surface)]"
          >
            홈케어·돌봄
          </Link>
        </div>
      </div>

      <section className="mt-10 border-t border-[var(--border)] pt-10">
        <h2 className="font-[family-name:var(--font-display)] text-xl font-semibold text-[var(--ink)]">
          채용 광고·제휴 문의
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-[var(--muted)]">
          미국 내·한국·원격 채용을 올리고 싶거나 광고·제휴를 문의하려면
          이메일로 연락해 주세요. 회사명, 직무, 근무지, 지원 링크, 희망 게재
          기간을 함께 보내 주시면 검수 후 안내드립니다.
        </p>
        <a
          href={mailHref}
          className="mt-5 inline-flex rounded-lg bg-[var(--brand)] px-5 py-3 text-sm font-semibold text-white hover:opacity-90"
        >
          {email}로 문의하기
        </a>
        <p className="mt-3 text-xs leading-relaxed text-[var(--muted)]">
          ArriveUS는 특정 고용·이민 결과를 보장하지 않으며, 게시 내용은 광고로
          표시될 수 있습니다.
        </p>
      </section>
    </div>
  );
}
